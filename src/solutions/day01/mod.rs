...
use self::decode::{Decode, Mode};

fn sum_decoded(lines: &[&str], mode: Mode) -> u32 {
    lines.iter().map(|&s| s.decode(mode)).sum()
}

fn part1(lines: &[&str]) {
    println!("Part 1: {}", sum_decoded(lines, Mode::DigitOnly));
}

fn part2(lines: &[&str]) {
    println!("Part 2: {}", sum_decoded(lines, Mode::DigitOrWord));
}
