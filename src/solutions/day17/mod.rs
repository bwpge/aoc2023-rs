...
use self::city::{City, Crucible, UltraCrucible};

fn part1(city: &City<Crucible>) {
    println!("Part 1: {}", city.traverse());
}

fn part2(city: &City<UltraCrucible>) {
    println!("Part 2: {}", city.traverse());
}

/// Executes the solution with provided input file.
pub fn exec<P: AsRef<Path>>(path: P) -> Result<()> {
    let city = City::from_file(&path)?;
    part1(&city);
    part2(&city.set_crucible::<UltraCrucible>());
