...
fn part1(records: &[Record]) {
    let sum: u64 = records.iter().map(Record::arrangements).sum();
    println!("Part 1: {sum}");
}

fn part2(records: &mut [Record]) {
    let sum: u64 = records.iter().map(|r| r.arrangements_unfold(5)).sum();
    println!("Part 2: {sum}");
}

/// Executes the solution with provided input file.
pub fn exec<P: AsRef<Path>>(path: P) -> Result<()> {
    let mut records = map_file_lines(path, Record::from_str)?;
