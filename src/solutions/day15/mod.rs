...
//! any remaining lenses as far forward in the box as they can go without
...
//!     lenses in the box, the new lens goes all the way to the front of the
//!     box.
//!
...
fn part1(steps: &[String]) {
    let mut h = Hasher::new();
    let sum = steps
        .iter()
        .fold(0, |value, s| value + h.combine_str(s).finalize());
    println!("Part 1: {sum}");
}

fn part2(steps: &[String]) {
    let mut map = FocalMap::new();
    let steps = steps
        .iter()
        .map(|s| Step::from_str(s.as_str()).expect("step should be valid"));
    map.execute_many(steps);

    println!("Part 2: {}", map.focusing_power());
}
