...
use self::galaxy::GalaxyMap;

fn distance_sum(map: &GalaxyMap) -> usize {
    map.galaxy_pairs()
        .into_iter()
        .fold(0, |value, (g1, g2)| value + map.distance(&g1, &g2))
}

fn part1(map: &mut GalaxyMap) {
    map.set_expansion(1);
    println!("Part 1: {}", distance_sum(map));
}

fn part2(map: &mut GalaxyMap) {
    map.set_expansion(1_000_000);
    println!("Part 2: {}", distance_sum(map));
}

/// Executes the solution with provided input file.
pub fn exec<P: AsRef<Path>>(path: P) -> Result<()> {
    let mut map = GalaxyMap::from_str(&std::fs::read_to_string(path)?)?;
